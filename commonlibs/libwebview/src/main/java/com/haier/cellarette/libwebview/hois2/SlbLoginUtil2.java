...
public class SlbLoginUtil2 {
    ...
    private static SlbLoginUtil2 sInstance;
    private static final Object lock = new Object();
    private Runnable mLastRunnnable;


    public SlbLoginUtil2() {
    }

    public static SlbLoginUtil2 get() {
        if (sInstance == null) {
            synchronized (lock) {
                sInstance = new SlbLoginUtil2();
            }
        }
        return sInstance;
    }
...
//        if (!TextUtils.isEmpty(SPUtils.getInstance().getString(CommonUtils.USER_TOKEN))) {
        if (!TextUtils.isEmpty(SPUtils.getInstance().getString("用户的TOKEN"))) {
            return true;
        }
